...
1. **Stale `tech-stack.md` is a self-inflicted footgun.** It says `deployment_target: cloudflare-pages`, but Cloudflare consolidated onto **Workers + Static Assets** for new projects, and **Astro 6 SSR is broken on Pages** (the `ASSETS` binding name is reserved on Pages). Future-you or an agent reading the doc could follow the wrong path and waste an evening debugging silent SSR failures. The repo's actual `wrangler.jsonc` is on the correct (Workers) path; only the doc is stale.
2. **`run_worker_first` gotcha for API auth routes.** Without `assets.run_worker_first: ["/api/*"]` in `wrangler.jsonc`, the Static Assets router shadows `src/pages/api/auth/*` and returns 1003/403. A subtle config trap a solo dev under timeline pressure can easily hit.
...
- **Logs**: `wrangler tail --status error --search "<query>" --format json` from the terminal. The agent can also read recent logs via the Cloudflare MCP observability server (`https://observability.mcp.cloudflare.com/mcp`) for structured tool-use access. Cloudflare also retains 24h of logs in the dashboard for visual inspection.

## Risk Register

| Risk | Source | Likelihood | Impact | Mitigation |
|---|---|---|---|---|
| `tech-stack.md` says `cloudflare-pages` — a future agent or future-self follows it and deploys to Pages, where Astro 6 SSR is broken | Devil's advocate | High (it's in the doc right now) | Medium (silent SSR failures, 1–2 evenings lost) | Update `context/foundation/tech-stack.md`: change `deployment_target: cloudflare-pages` → `cloudflare-workers`. Add a one-line note in `AGENTS.md` clarifying Workers is the target. |
| API auth routes return 1003/403 because Static Assets router shadows them | Devil's advocate / Research | Medium | High (auth flow blocked) | Verify `wrangler.jsonc` includes `assets.run_worker_first: ["/api/*"]` (or equivalent). Test `/api/auth/signin` after each deploy with a curl smoke check in CI. |
| 30s CPU ceiling blocks future fan-out / bulk-import features | Devil's advocate / Pre-mortem | Medium (only if scope creeps beyond MVP) | Medium | For MVP scope (tag-set intersection over ≤100s of books per user), this is fine. For v2 ideas, plan to use Durable Objects or Queues — document the constraint in `idea-notes.md` to prevent surprise. |
| `workerd` ≠ Node — runtime failure on Node-only npm deps | Devil's advocate | Low–Medium | Medium | Audit dependencies before adding: check for Node-only API usage. Prefer adapters with explicit Workers/edge support (e.g., `@supabase/supabase-js` — already chosen). |
...
1. **Fix the stale doc.** Update `context/foundation/tech-stack.md`: change `deployment_target: cloudflare-pages` → `deployment_target: cloudflare-workers` (or `cloudflare-workers-static-assets` for precision). Add a one-line note in `AGENTS.md` under "Hard Rules" clarifying that the project targets Workers (not Pages).
2. **Audit `wrangler.jsonc` for the API-shadowing gotcha.** Confirm `assets.run_worker_first: ["/api/*"]` (or the project's equivalent path pattern) is set. If absent, add it and redeploy. Smoke-test `/api/auth/signin` returns the expected response, not 1003/403.
...
7. **Install the Cloudflare MCP servers in Cursor.** Add `mcp.cloudflare.com/mcp` (Code Mode) and `observability.mcp.cloudflare.com/mcp` to `.cursor/mcp.json` for agent-driven log inspection and deploy triage.
