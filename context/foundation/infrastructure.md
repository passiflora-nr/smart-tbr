...
- **Production deploys**: Push to `main` → GitHub Actions `ci` job (lint → `npm test` → `npm run test:e2e` → build) → `deploy` job (`cloudflare/wrangler-action@v3`, post-deploy curl smoke checks). Manual redeploy: Actions → **CI** → **Run workflow** (`workflow_dispatch`). Local emergency deploy: `npm run build && npx wrangler deploy` (prefer CI for routine changes).
...
- **Secrets**: `SUPABASE_URL`, `SUPABASE_KEY` (anon / publishable), and `SUPABASE_SERVICE_ROLE_KEY` (secret / `service_role`; account deletion only) are declared `context: "server"`, `access: "secret"` in `astro.config.mjs`. Local `.dev.vars` (read by workerd in `npm run dev`) holds **`SUPABASE_URL` and `SUPABASE_KEY` only** — do not put the admin key there. `SUPABASE_SERVICE_ROLE_KEY` is a production Worker secret (`wrangler secret put`, refreshed by CI's `wrangler-action` `secrets:` input). GitHub repo secrets: `SUPABASE_URL`, `SUPABASE_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`. Rotation: regenerate in Supabase, update GitHub secrets, then re-run deploy (or `wrangler secret put` locally). **Do not** let the Cloudflare Supabase integration inject a service-role value into `SUPABASE_KEY` — that would disable Row-Level Security across the whole app.
...
| Surprise Workers paid bill if traffic spikes past free tier unnoticed                                                                | Unknown unknowns                         | Low (MVP traffic is small)                             | Low ($5–$20 unexpected)                                      | **Done:** Workers usage alert set at 80k req/day (owner-confirmed 2026-09-06).                                                                                                                            |
...
6. ~~**Set a usage alert.**~~ **Done** (owner-confirmed 2026-09-06). Cloudflare usage notification at ~80k req/day.
7. ~~**(Optional) Install the Cloudflare MCP servers in Cursor.**~~ **Skipped** — user opted out; use `wrangler tail` and the dashboard.

Post-rollout dashboard hygiene is done (owner-confirmed 2026-09-06): production sign-up → sign-in smoke, `wrangler tail` while clicking around, usage alert, and ops bookmarks. Dashboard links: [Worker](https://dash.cloudflare.com/10e6c5de7ae20000c186703ad894eab2/workers/services/view/smart-tbr) · [Supabase Auth users](https://supabase.com/dashboard/project/kahvpxeygnmqpysrskok/auth/users). Archived checklist: [`deploy-plan.md`](../archive/deploy-plan.md#remaining-manual-ops-post-rollout).
...
- ~~CI/CD pipeline setup~~ — **Done (2026-06-11):** `.github/workflows/ci.yml` runs lint, `npm test`, `npm run test:e2e`, and build on PR/push; `deploy` job auto-deploys on push to `main` via `wrangler-action@v3` with post-deploy smoke checks
