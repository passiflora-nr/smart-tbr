...
**Contract**: Takes the array of server-returned book rows (typed from `Tables<"books">` in `src/lib/database.types.ts`, whose helpers are currently unused) and renders each with title, author, trope chips, and description when present, newest first, with a count.
