...
Unrelated pre-existing issue observed during review, not introduced by this plan: a direct `tsc --noEmit` reported a type error at `tests/integration/support/local-services.ts` (`ChildProcessByStdio` not assignable to `ChildProcessWithoutNullStreams`). Resolved in `555b652` (introduced `AstroDevProcess`) and kept compatible with Playwright's `surviveParentExit` mode via overloads on `startAstroDev` and `ChildProcess` on the public handle.
